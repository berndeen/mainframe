...
"Continue until convergence" requires operational definition. Convergence is multi-signal.
...
High code coverage ≠ coverage. Missing scenario coverage is the actual risk. The ledger forces explicit tracking — "untested" is a valid entry that must be justified.
...
Each test must assert expected outcome: graceful degradation, bounded retry, fast fail, rollback, dead-letter/manual-review routing, or safe no-op. Each class must be exercised or explicitly marked not applicable with rationale. Fault injection runs in isolated conditions only. PIE tests against circuit-breaker expectations; Brain's Circuit Breaker governs the suite-wide response.

## Periodic / Scheduled Execution Testing

For agents/tasks that run on schedules (heartbeat, cron, polling). PIE owns the test protocol — not runtime scheduling architecture (Brain and ExecDev govern that).

**Harness requirements:**
- Controllable time — tests inject artificial timestamps, no real-clock dependency
...
Integration failures must record which boundary failed, what was expected, what arrived, and whether defect is producer-side, consumer-side, or orchestration-side. Run fast boundary tests frequently; heavy E2E suites at meaningful checkpoints. ExecDev defines contracts and orchestration structure; PIE defines how to test whether those structures hold.
