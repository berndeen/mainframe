...
# The Brain — Operational Governance Layer v4.4
...
Optional: `mission_ref: {path to ExecDev goal ancestry}` — include when organizational hierarchy exists. Absence is not a gate.

Non-HIGH confidence → watchlist items become active monitoring obligations.

Purpose informs prioritization when budget is constrained — higher purpose = higher priority in resource allocation.
...
Budget = declared resource ceiling for a task/swarm/session. Brain defines decision logic; platforms implement tracking.

### Declaration
Declare at task start — at minimum one dimension:
```
Budget: {scope} | Ceiling: {amount} | Unit: {tokens|steps|minutes|API-calls} | Warn: 80% | Hard: 100%
```
- STRATEGIC tasks and swarms/councils: MANDATORY
- STANDARD tasks: optional, inherit from parent if set
- QUICK tasks: no budget overhead

<HARD-GATE>
At every HARD-GATE: verify remaining >= estimated cost to complete.
Warning at 80%: log, continue with monitoring.
Hard stop at 100%: STOP. Report partial results with "budget-stopped" status. Escalate to parent/human.
Agent may NOT self-authorize budget extension. Parent extends with written rationale.
</HARD-GATE>

### Delegation
- Parent retains reserve; allocates portion to each child
- Sum of child allocations ≤ parent ceiling
- Delegation brief includes child's allocated portion

### Platform Integration
- Brain: WHAT to check and WHEN to stop
- Platform: HOW to measure and track
- No platform tracking → agents self-report at gates (honor system with audit obligation)

## Audit Events

Governance decisions produce append-only audit events. Path: `governance/audit/{task-id}.jsonl`. Once written, never modified or deleted. Distinct from ADRs (mutable workflow documents) and telemetry (summary metrics).
...
- Budget threshold breaches: warning or hard-stop
- Circuit breaker trips: trigger, scope

### Strategic-Only Events
- SWOT completion: task, recommendation
- Delegation assignments: council/swarm composition
- ADR creation: ADR-id, status
- Human escalations: reason, resolution

### Schema
```json
{ "timestamp": "ISO-8601", "event_type": "classification|transition|gate|budget|circuit|swot|delegation|adr|escalation", "task_id": "id", "agent": "id", "mode": "QUICK|STANDARD|STRATEGIC", "detail": {}, "confidence": "high|medium|low" }
```

Append-only: corrections create a new event referencing the original sequence. No overwrite, no delete.

**Cost scoping:** Quick ~3 events. Standard ~5. Strategic ~10+. Audit overhead must not dominate task cost.

## Heartbeat Governance

For periodic/scheduled agents. Pre-classification gate runs BEFORE Task Classification Protocol.

### Pre-Classification Gate (Heartbeat Only)
Before classifying, evaluate:
1. Has triggering condition changed since last run? NO → skip.
2. Is remaining budget sufficient for another cycle? NO → skip.
3. Does pending work exist? NO → skip.

All three NO → emit heartbeat audit event, sleep. At least one YES → proceed to classification normally.

### Zombie Prevention
- Max-idle threshold (default: 5 consecutive no-action cycles)
- Exceeding max-idle → escalate to parent: `"Agent {id} idle for {n} cycles. Continue/suspend?"`
- Suspended agents stop until explicitly resumed

### Budget Integration
- Declare per-cycle budget AND cumulative period budget
- Pre-classification gate consumes per-cycle; actual work consumes period budget
...
- Budget gaming: splitting tasks to avoid per-task ceilings
- Audit theater: emitting events without meaningful detail
- Retry storms: retrying failures without backoff or circuit breaker check
- Zombie heartbeats: periodic agents running indefinitely with no work
...
| "Budget slows us down" | Unbounded cost is not speed — it's uncontrolled burn. Budget forces prioritization, which IS the work. |
...
States: `CLOSED` (normal) → `OPEN` (tripped) → `HALF-OPEN` (testing) → `CLOSED` (recovered)

Triggers (any one sufficient):
1. 3+ same-class failures on same operation within one task
2. Cascading failure crosses 2+ tier boundaries
3. Retry spend exceeds 50% of task budget without progress
4. Recursive delegation loop: agents passing same error back and forth

Response:
1. OPEN: pause the failing operation class (not all operations)
2. Log: operation, trigger, agents involved, cost consumed
3. Escalate to next tier with full failure context
4. HALF-OPEN: after review, allow single test retry
5. Succeeds → CLOSED. Fails → remain OPEN.

Integrates with Budget Protocol (retry drain = trigger) and Audit Events (circuit breaks are mandatory events).

## Gate Response Protocol

Every HARD-GATE must include a remediation hint. A gate without remediation guidance is incomplete.

Format: `BLOCKED: {what failed} → REMEDIATE: {what to do next}`

Examples:
- `BLOCKED: No classification → REMEDIATE: Run Task Classification Protocol before executing`
- `BLOCKED: Budget exhausted → REMEDIATE: Report partial results, request budget extension from parent`
- `BLOCKED: SWOT missing → REMEDIATE: Complete SWOT before submitting proposal`
