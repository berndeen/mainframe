...
Delegation briefs include ancestry scoped by tier — prevents context waste and scope drift.
...
- `Purpose` field (from Brain) references goal ancestry via `mission_ref` — ExecDev defines hierarchy; Brain supplies the hook.
- Tier-appropriate ancestry depth only. Passing corporate mission to a T3 = context waste + hallucination risk.
...
1. **Discovery** — scan workspace/registry for skills matching capability gap
2. **Resolution** — resolve `skillDependencies`, check version compatibility, detect conflicts; newest compatible wins on conflict
3. **Injection** — load skill into agent context, apply routing protocol
4. **Execution** — skill governs within domain; agent operates under injected constraints
5. **Ejection** — remove skill from context when task completes; free tokens for next workload
6. **Graceful Degradation** — skill unavailable (missing, offline, version conflict): base tools sufficient? YES → proceed, emit explicit warning. NO → halt and escalate. Reduced capability over silent failure.
...
- `nack`: receiver rejects (bad input, out of scope) — sender reroutes or escalates
- `dead-letter`: TTL expired, no acknowledgment — escalate to orchestrator
- Priority governs processing order, not delivery order
- Every `dispatched` requires `acknowledged` or `nack`. No silent drops.
...
JSON manifest for skill packages. Every skill ships with `skill.json`. Schema-first distribution — all tooling (CLI, resolution, composition) runs from this schema.
...
**Resolution rules:**
- `skillDependencies`: resolved before injection; topological sort, deduplication across registries; newest compatible wins on conflict; hard conflict → halt + escalate
- `composesWith`: peer skills co-loaded; no hard dependency
- `dependencies`: runtime/npm packages (non-skill)
- `target`: maps abstract path to concrete location; supports local override
- `meta`: arbitrary extensibility without schema changes — use for `compressed`, `word_count`, `stability`, `compliance_level`
...
Pre-load inspection protocol. Agent inspects impact without committing.

**Command:** `skill preview {skill-name}` → impact summary: estimated context window cost (tokens), files touched, dependencies pulled, routing modules available.

**Decision:** Agent proceeds, defers, or requests alternative. Prevents surprise context blowout from large skill injection. Summary is terse — one line per dimension. Not a report, a gate.

---

## Context Isolation Pattern

For swarms across multiple contexts (clients, projects, environments):
...
- **Secret scoping:** secrets namespaced `context_{id}_{key}` — credentials scoped to matching context only
- **Cross-context prohibition:** no direct path reference across boundaries; inter-agent communication protocol only
