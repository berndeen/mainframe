...
# Executive Dev Architecture v3.3
...
Every delegation brief includes ancestry scoped by tier — prevents context waste and scope drift.

- **T1:** mission → initiative → objective (full strategic context required)
- **T2:** initiative → objective → task (pipeline-aware; no mission context)
- **T3:** objective → task (execution scope only; no initiative or mission)
- `Purpose` field (from Brain) references goal ancestry via `mission_ref` — ExecDev defines the hierarchy; Brain supplies the hook.
- Delegation briefs include only tier-appropriate ancestry depth. Passing corporate mission to a T3 executor = context waste + hallucination risk.
...
CONTRACT:
  ...
  BUDGET:
    per_task_ceiling: {amount, unit}
    per_period_ceiling: {amount, unit, period}
    budget_unit: tokens | api_calls | wall_clock_minutes | credits
    overage_policy: hard_stop | warn_and_continue | escalate
```

Versioned. Breaking changes require ADR + migration. No agent without contract. No contract without review. Budget fields integrate with Brain's Budget Protocol — Brain defines governance, contract defines allocation.

### Runtime Skill Injection Lifecycle

Skills are transient context, not permanent identity. Six phases:

1. **Discovery** — scan workspace/registry for available skills matching capability gap
2. **Resolution** — resolve `skillDependencies`, check version compatibility, detect conflicts; newest compatible version wins on conflict
3. **Injection** — load skill into agent context, apply routing protocol
4. **Execution** — skill governs within its domain; agent operates under injected constraints
5. **Ejection** — remove skill from context when task completes; free tokens for next workload
6. **Graceful Degradation** — if skill unavailable (missing, offline, version conflict): can outcome be achieved via base tools? YES → proceed, emit explicit warning. NO → halt and escalate. Agent continues at reduced capability rather than silent failure.
...
```json
{
  ...
  "ttl": "duration — message expires after",
  "status": "dispatched | acknowledged | processing | completed | nack | dead-letter",
...
- `nack`: receiver rejects message (bad input, out of scope) — sender must reroute or escalate
- `dead-letter`: TTL expired with no acknowledgment — escalate to orchestrator
- Priority governs processing order, not delivery order
- Every `dispatched` message requires `acknowledged` or `nack`. No silent drops.
...
JSON manifest for skill packages. Every skill ships with a `skill.json` manifest. Schema-first distribution — all tooling (CLI, resolution, composition) runs from this schema.
...
```json
{
  ...
  "type": "skill:governance | skill:architecture | skill:implementation | skill:reference | skill:template | skill:audit",
  "title": "Human-readable title",
  "description": "One-line description",
  "author": "string",
  "license": "string",
  "tier_compatibility": ["T1", "T1-Super", "T2", "T3"],
  "dependencies": [],
  "skillDependencies": ["skill-name@^version"],
  "composesWith": ["skill-name@^version"],
  "files": [
    { "path": "string", "type": "skill:core | skill:audit | skill:config", "target": "optional override path" }
  ],
  "routing": {
...
| `skill:reference` | Domain knowledge, canonical URLs | (future) |
...
**Resolution rules:**
- `skillDependencies`: resolved before injection; topological sort, deduplication across registries; newest compatible version wins on conflict; hard conflict → halt + escalate
- `composesWith`: peer skills that may be co-loaded; no hard dependency
- `dependencies`: runtime/npm packages (non-skill)
- `target` field: maps abstract path to concrete project location; supports local override
- `meta` field: arbitrary extensibility without schema changes — use for `compressed`, `word_count`, `stability`, `compliance_level`
- Skills are data-only (text/markdown) — no arbitrary code execution. Trust model: copy-don't-install.

---

## Skill Impact Preview

Pre-load inspection protocol. Before injecting a skill, agent inspects impact without committing.

**Command:** `skill preview {skill-name}` → returns impact summary: estimated context window cost (tokens), files touched, dependencies pulled, routing modules available.

**Decision:** Agent proceeds, defers, or requests alternative. Prevents surprise context window blowout from large skill injection. Impact summary is terse — one line per dimension. Not a report, a gate.

---

## Context Isolation Pattern

When a swarm operates across multiple contexts (clients, projects, environments):

- **Workspace partitioning:** `workspace/{context_id}/...` — agents jailed to context directory; no cross-boundary file access
- **State jailing:** state stores require `context_id` as primary partition key; reads/writes bounded to context
- **Secret scoping:** secrets namespaced `context_{id}_{key}` — credentials available only to agents within matching context
- **Cross-context prohibition:** no direct path reference across boundaries; use inter-agent communication protocol only
- **Context lifecycle:** create → active → frozen → archived

Isolation must be native to architecture — bolting on later is catastrophic. Define `context_id` at swarm initialization, propagate through every delegation brief.
