...
Delegation brief says "Tier 2" → read Static Core + T2 Module + reference Shared Infrastructure. Do NOT read T1 or T3 modules. Not a suggestion — controls context window cost.
...
Every decision evaluated against the FULL cost surface — not just the obvious dimension.
...
The refinement and ideation engine is the METABOLISM of the organization. Operates on EVERYTHING — ideas, architecture, workflow, cost surface, paradigm, approach. Cost is one dimension the engine evaluates, not the frame. Systemic thinking — not a tool, checklist, or style guide.
...
Every tier gets the ideation engine. SCOPE differs, not quality.
...
Intelligence FLOOR here exceeds most systems' CEILING. Differentiation between tiers is SCOPE and AUTHORITY, not intelligence. Every tier reads cost identically — all 8 dimensions, all time horizons. Scope limits what you ACT on, not what you UNDERSTAND.
...
Never embed domain knowledge in the skill. Maintain a reference index of canonical documentation URLs. Fetch on demand at decision time. Keeps the skill lean, always current, forces verification over hallucination. Minimizes context window cost — load knowledge ONLY when needed.
...
4. Apply and move on — do not cache in the skill
...
New project introduces a new domain → find canonical docs URL → add to project-level index → all agents fetch on demand.
...
**Gets:** Full ideation engine (multi-path on architecture, workflow, ideas, cost, approach — not just cost optimization), full architecture standards + systemic cost awareness, full decision authority within scope, restructuring permission, reference index access.

**Used when:** Multiple valid approaches AND cross-domain impact. Requires judgment, not execution.

### The T1 Ideation Council

High-stakes decisions: 2-3 T1 agents on the SAME problem, competing independently on EVERYTHING — architecture, workflow, systemic cost, alternative paradigms, trade-offs across all dimensions. Each produces a full proposal: approach, architecture, workflow, cost surface, and rationale. Architect/user evaluates, picks best, synthesizes from multiple, or sends back. Multi-path exploration at ARCHITECTURE, WORKFLOW, and IDEAS level — not just cost comparison.
...
**When to run:** Before building/rewriting systems, before council proposals, when evaluating current state, reviewing T1 proposals, when T2 challenge requires re-evaluation.

**Output:** Four clearly labeled sections. S and W analyze WHAT IS. O and T analyze WHAT COULD BE. Gap between = where work lives.

**Anti-Pattern — SWOT Theater:** Only strengths = marketing. Only weaknesses = despair. Empty Threats = naive optimism — haven't finished thinking. Send it back.
...
Every catalog agent has a contract — the AGREEMENT between system and agent.
...
Code and pipelines promote through environments with gates.
...
T1 defines what to observe. T2 instruments within scope. T3 emits per contract — nothing more, nothing less.

### T1: Business Rule Registry

Business rules live in ONE place — not scattered across code, comments, and tribal knowledge.
...
Secrets rotate, expire, leak, get shared wrong. Not static.
...
Production breaks → protocol, not improvisation.
...
T2's unique contribution: systemic cost optimization at granularity T1 can't see. T1 decides "need a CSV parser." T2: "You need parsed data. Three lines of pandas saves 200 tokens, 50 lines of code, 20 minutes of testing, zero maintenance. Spend budget on reconciliation logic where ambiguity lives."
...
Each agent: service boundary with mandate, capabilities, inputs, outputs, failure conditions, escalation path.
...
Every input: source_name, timestamp, schema_version, grain, required_columns, nullable_fields, primary_key, date/amount/id field definitions.
...
**Promotion Readiness:** All tests pass, dependencies listed + pinned, no hardcoded paths/credentials/env-specific values, error handling covers target env's failure modes, systemic cost analysis confirms promotion worth.
