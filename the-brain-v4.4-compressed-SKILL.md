...
Non-HIGH confidence → watchlist items become active monitoring obligations. Purpose informs prioritization when budget is constrained — higher purpose = higher priority in resource allocation.
...
- STANDARD: optional, inherit from parent if set
- QUICK: no budget overhead
...
Append-only governance events. Path: `governance/audit/{task-id}.jsonl`. Never modified or deleted. Distinct from ADRs (mutable) and telemetry (summary).
...
Append-only: corrections create a new event referencing the original. No overwrite, no delete.
...
1. Triggering condition changed since last run? NO → skip.
2. Remaining budget sufficient for another cycle? NO → skip.
3. Pending work exists? NO → skip.
...
Integrates with Budget Protocol (retry drain = trigger) and Audit Events (circuit breaks = mandatory events).

## Gate Response Protocol

Every HARD-GATE must include a remediation hint. Gate without remediation = incomplete.
